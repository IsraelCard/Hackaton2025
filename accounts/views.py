...
def home(request):
    ...
    for dispositivo in dispositivos.filter(estado=True):
        if dispositivo.consumo:
            consumo_diario = dispositivo.consumo * 24  # 24 horas al día
            consumo_total['diario'] += consumo_diario
            consumo_total['mensual'] += consumo_diario * 30  # 30 días
    
    consumo_total['kwh'] = consumo_total['mensual'] / 1000  # Convertir a kWh
